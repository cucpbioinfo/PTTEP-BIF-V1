...
export const AssetDensityBar = ({SpeciesId}:Speciesprops) => {
  interface IProps {}
  interface IState {}
  var ThisSpeciesName ="";
  ...
  const [assetdensity, setAssetDensity] = useState([])
  ...
  const fetchAssetDensity = async () => {
    const {
      assetId,
      year,
    } = router.query
    const { data } = await listAssetDensity({
  ...
  // useEffect(() => {
  //   fetchAssetDensity(router.query.speciesId as string)
  // }, [router.query.speciesId])
  ...
  assetdensity.map((density) => 
    (
      
      ThisSpeciesName = density?.speciesName,
      DataXStation.push(density?.assetName),
  ...
  // console.log("data : " + DataXSurface )
  // console.log("data : " + DataXZone )
  class BarAssetDensity extends React.Component<IProps, IState> {
    getOption = () => ({
      title: {
